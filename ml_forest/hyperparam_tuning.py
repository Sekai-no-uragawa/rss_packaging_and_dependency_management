...
def tuning(
    ...
    with mlflow.start_run(run_name=f'tuning_{clf_type}') as run:
        ...
        for train_ix, test_ix in cv_outer.split(X):
            ...
            space = dict((clf_type + '__' + x, ast.literal_eval(y)) for x, y in config['PARAMS'].items())
            ...
            print('>acc=%.3f, est=%.3f, f1=%.3f, roc_auc=%.3f, cfg=%s' % (acc, result.best_score_, f1, roc, result.best_params_))
